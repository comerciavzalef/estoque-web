// ══════════════════════════════════════════════════════════════
//  ESTOQUE DIGITAL — app.js v13.1 (God Mode Telemetry Integrated)
...
// ── Restaura sessão se já estava logado (espera DOM + tracker) ──
(function () {
...
// ── Inicializa telemetria God Mode (init apenas; loginSuccess vai em iniciarApp) ──
document.addEventListener('DOMContentLoaded', function () {
...
function logout() {
  // ── Telemetria: logout ANTES de zerar sessão ──
  if (window.GodModeTracker) {
...
function iniciarApp() {
  // ── Telemetria: notifica sessão ativa (login novo OU restaurada) ──
...
function syncDados() {
  fetch(API_URL + '?sync=1')
    ...
    .catch(function (err) {
      setBadge(false);
      // ── Telemetria: falha ao sincronizar ──
      if (window.GodModeTracker) {
        GodModeTracker.log({
          tipo: 'ALERTA',
          mensagem: 'Falha ao sincronizar estoque: ' + (err && err.message ? err.message : 'rede')
        });
      }
    });
}
  ...
  produtos.forEach(function (p) {
    ...
    if (!grupos[chave]) {
      grupos[chave] = {
        ...
        quantidadeTotal: 0, melhorStatus: 'OK', melhorDias: 99999,
        lotes: [], linhas: []
      };
...
function salvarEdicao() {
  ...
  fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(payload), redirect: 'follow' })
    ...
    .catch(function (err) {
      toast('Sem conexão');
      // ── Telemetria: erro ao editar ──
      if (window.GodModeTracker) {
        GodModeTracker.log({
          tipo: 'ERRO',
          mensagem: 'Falha ao editar produto: ' + (err && err.message ? err.message : 'rede')
        });
      }
    })
...
function confirmarSaidaLote() {
  ...
  fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ acao: 'saidaLote', colaborador: sessao.nome, nome: sessao.nome, itens: itensPayload }), redirect: 'follow' })
    ...
    .catch(function (err) {
      ...
      // ── Telemetria: falha na baixa ──
      if (window.GodModeTracker) {
        GodModeTracker.log({
          tipo: 'ERRO',
          mensagem: 'Falha ao confirmar saída em lote: ' + (err && err.message ? err.message : 'rede')
        });
      }
    })
    ...
    .then(function (d) {
      if (d.status === 'ok') {
        ...
        if (d.match) { showSuccess('✅', 'Tudo Certo!', d.msg); }
          ...
          // ── Telemetria: divergência detectada ──
          if (window.GodModeTracker) {
            GodModeTracker.log({
              tipo: 'ALERTA',
              mensagem: 'Divergência de auditoria — Linha ' + linha + ': físico=' + qtd + ', sistema=' + (qtd - d.diferenca)
            });
          }
...
function enviarEntrada() {
  ...
  fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(payload), redirect: 'follow' })
    ...
    .catch(function (err) {
      ...
      // ── Telemetria: falha ao salvar entrada ──
      if (window.GodModeTracker) {
        GodModeTracker.log({
          tipo: 'ERRO',
          mensagem: 'Falha ao salvar entrada: ' + (err && err.message ? err.message : 'rede')
        });
      }
    })
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', function () {
    navigator.serviceWorker.register('./sw.js', { scope: './' })
      .then(function (reg) {
        console.log('[Estoque] Service Worker registrado:', reg.scope);
      })
      .catch(function (err) {
        console.warn('[Estoque] Service Worker falhou:', err);
      });
  });
}
